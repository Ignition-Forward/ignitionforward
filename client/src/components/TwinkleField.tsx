  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      starsRef.current.forEach(star => {
        ...
        // Outer soft glow (gold tinted)
        const glowSize = star.size * 4;
        const glow = ctx.createRadialGradient(
          star.x, star.y, 0,
          star.x, star.y, glowSize
        );
        glow.addColorStop(0, `rgba(255, 250, 240, ${currentOpacity * 0.8})`);
        glow.addColorStop(0.15, `rgba(255, 240, 220, ${currentOpacity * 0.4})`);
        glow.addColorStop(0.4, `rgba(201, 169, 98, ${currentOpacity * 0.15})`);
